class Particle {
  constructor(x, y, type = "mouse") {
    ...
    this.baseSize =
      type === "mouse" ? Math.random() * 2 + 1 : Math.random() * 4 + 2;
    ...
    this.speedY =
      type === "mouse" ? Math.sin(angle) * speed : -Math.random() * 4 - 2; // Upward bias for typing particles
  ...
  update() {
    ...
    if (lifeRatio > 0.8) {
      // Initial pop effect
      this.size =
        this.baseSize * (1 + Math.sin((1 - lifeRatio) * Math.PI) * 0.5);
    } else {
      // Gradual shrink
...
class ParticleSystem {
  ...
  getCursorPosition(element) {
    ...
    if (
      ...
      return {
        x:
          rect.left +
          Math.min(element.selectionStart, element.value.length) * 8,
  ...
  handleKeydown(e) {
    if (e.key.length > 1 && !["Enter", "Space", "Backspace"].includes(e.code))
      return;
